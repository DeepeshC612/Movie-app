...
export default function EditMovie() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, setValue, setError, reset } = useForm<MovieForm>({
  ...
  const fetchMovie = async () => {
    try {
      ...
      if (data.status) {
        const movie = data.result.movies.find((m: any) => m.id === parseInt(movieId));
        if (movie) {
          reset({
            title: movie.title,
  ...
  const onSubmit = async (data: MovieForm) => {
    try {
      ...
      showToast('Failed to update movie: ' + error, 'error');
    } finally {
      setUploading(false);
    }
  };
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col lg:flex-row gap-6 lg:gap-8">
            ...
            <div className="w-full lg:flex-1">
              ...
              >
                {image ? (
                  ...
                  <div className="w-full h-full relative">
                    <img src={currentPoster} alt="Current poster" className="w-full h-full object-cover rounded-lg" />
                    <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                      <p className="text-white text-sm">Click to change image</p>
                    </div>
                  </div>
            ...
            <div className="w-full lg:flex-1 space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4 lg:pt-8">
                <button type="button" onClick={handleCancel} className="w-full sm:flex-1 h-12 font-semibold text-white border border-white/30 rounded-xl transition-all duration-200 hover:bg-white/10 text-sm sm:text-base leading-6">
                  Cancel
                </button>
                <button type="submit" disabled={isSubmitting || uploading} className="w-full sm:flex-1 h-12 font-semibold text-white rounded-xl transition-all duration-200 hover:transform hover:-translate-y-0.5 hover:shadow-lg active:transform active:translate-y-0 text-sm sm:text-base leading-6 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer" style={{ backgroundColor: '#2BD17E', boxShadow: '0 4px 12px rgba(43, 209, 126, 0.3)' }}>
                  {isSubmitting || uploading ? 'Updating...' : 'Update'}
                </button>
              </div>
            </div>
          </form>
